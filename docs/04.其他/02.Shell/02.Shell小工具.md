...
# 捕获终止信号，确保 clean exit
trap 'echo "收到终止信号，停止监控..."; kill $PID 2>/dev/null || true; exit 0' SIGINT SIGTERM
...
while true; do
    ...
    if ! kill -0 "$PID" 2>/dev/null; then
        echo "[$(date)] 错误：无法启动命令，PID $PID 无效"
    ...
    echo "[$(date)] 进程 $PID 退出，退出代码: $EXIT_CODE"

    # 统计连续失败次数
    if [ $EXIT_CODE -ne 0 ]; then
        RETRY_COUNT=$((RETRY_COUNT + 1))
    else
        RETRY_COUNT=0  # 成功运行后重置计数器
    fi

    # 判断是否超过最大重试次数（可选）
    if [ $RETRY_COUNT -ge $MAX_RETRIES ]; then
        echo "[$(date)] 错误：连续失败 $MAX_RETRIES 次，停止尝试以避免风暴"
        exit 1
    fi

    # 重启前等待
    echo "[$(date)] $CHECK_INTERVAL 秒后尝试重启..."
    sleep "$CHECK_INTERVAL"
done
